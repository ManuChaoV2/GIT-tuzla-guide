...
fn pre_upgrade() {
    let counter = get();
    ic_cdk::storage::stable_save((counter,)).expect("Saving state to stable storage failed");
}

#[ic_cdk_macros::post_upgrade]
fn post_upgrade() {
    let (counter,): (Nat,) =
        ic_cdk::storage::stable_restore().expect("Restoring state from stable storage failed");
    set(counter);
}
